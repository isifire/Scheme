...
;          Tesis: addLast(l, l1, l2, ...) = cons(append(car(resto)), H)

(define (addLast l . resto)
  (cond [(null? resto) resto]
        [else (cons (append (car resto) l)
                    (apply addLast (cons l (cdr resto))))]))
...
(define (addLast-FOS l . listas)
 (map (lambda (lista) (append lista l)) listas)
 )
...
(define (addLast+ ll . listas)
  (addLast-FOS ll listas)
 )
                                           ;(( () (d) (1 (2 3)) (a) ((b) c) ))
(display "addLast+: ")
...
(cadar (filter list? (map (lambda (persona) (member nombre persona)) manos)))
)
...
(define (comodin? cartas-mano)
 (if (null? (filter list? (map (lambda (carta) (or (member 'comodin-roba4 carta) (member 'comodin-color carta))) cartas-mano))) #f #t) 
 )
...
(filter (lambda (carta) (or (equal? valor (car carta)) (not(number? (car carta))) )) cartas-mano) 
)
...
(define (color-compatible color cartas-mano)
 (filter (lambda (carta) (or (equal? (list color) (cdr carta)) (not(number? (car carta))) )) cartas-mano)
)
...
(define (compatibles carta-o cartas-mano)

(filter (lambda (carta) (or
                         (not(number? (car carta))) 
                         (equal? (car carta-o) (car carta))    
                         (equal? (cdr carta-o) (cdr carta)))
                         ) cartas-mano)
)


(display "compatibles: ")
(compatibles '(9 rojo) (cartas-mano 'ana Manos)) ;=> ((4 rojo) (9 rojo) (9 verde) (comodin-color) (9 verde))
(display "compatibles: ")
(compatibles '(comodin-color verde)(cartas-mano 'maria Manos)) ;=> ((7 verde) (comodin-roba4) (3 verde) (8 verde))
...
(map
 (lambda (persona) (car persona))
 (filter (lambda (carta) (or
                          (member '(comodin-color) (cadr carta))
                          (member '(comodin-roba4) (cadr carta)))) manos))
...
(filter (lambda (persona) (or
                           (equal? (car carta) (lambda (atributo) (car atributo) persona) )
                           ;(equal? (cdr carta) (cdaadr persona))
                           )) manos)
