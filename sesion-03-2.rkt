...
;       Tesis    : Extrae(Datos, Filtro, Formato) =
;                                  si Filtro(car(Datos))
...
;a) Los nombres de las personas adultas
...
;b) La lista de nombres completos de todas las personas
(display "TODOS LOS NOMBRES COMPLETOS: ")
(Extrae Datos (lambda (x) (> (get-edad x) 0)) get-nombre-completo)


;c) Los nombres de todas las mujeres que trabajan
(display "NOMBRES TODAS LAS MUJERES TRABAJAN: ")
(Extrae Datos (lambda (x) (equal? (cons (get-genero x) (trabaja? x)) '(M . #t))) get-nombre)


;d) Los nombres completos de todas las personas que han estudiado INFORMATICA
(display "NOMBRES COMPLETOS TODOS LOS QUE ESTUDIAN INFORMATICAS: ")
(Extrae Datos (lambda (x) (equal? (get-estudios x) '(INFORMATICA) ))get-nombre-completo)


;e) Los pares (género edad) de todas las personas sin estudios
(display "GENERO-EDAD SIN ESTUDIOSS: ")
(Extrae Datos (lambda (x) (equal? (get-estudios x) '())) (lambda (y) (cons (get-genero y) (get-edad y))))


;f) La edad de todas las personas que han estudiado INFORMATICA
(display "EDAD INFORMATICOS: ")
(Extrae Datos (lambda (x) (equal? (get-estudios x) '(INFORMATICA) ))get-edad)
...
;a) Los nombres de las personas adultas
(display "Nombres de adultos")
(Extrae Datos (gen-filtro (lambda (x) (get-edad x)) < 18) get-nombre)



;b) La lista de nombres completos de todas las personas
(display "Nombres completos de todos: ")
(Extrae Datos (gen-filtro (lambda (x) (get-edad x)) < 0) get-nombre-completo)



;c) Los nombres de todas las mujeres que trabajan
(display "Nombres TODAS las mujeres que trabajan")
(Extrae Datos (gen-filtro (lambda (x) (cons (get-genero x)(trabaja? x) ) ) equal? '(M . #t)) get-nombre)



;;d) Los nombres completos de todas las personas que han estudiado INFORMATICA
(display "Personas que estudian informatica")
(Extrae Datos (gen-filtro (lambda (x) (get-estudios x)) member 'INFORMATICA) get-nombre-completo)



;e) Los pares (género edad) de todas las personas sin estudios
(display "Los pares GENERO.EDAD de personas sin estudios: ")
(Extrae Datos (gen-filtro (lambda (x) (get-estudios x)) equal? '()) (lambda (y) (cons (get-genero y) (get-edad y))) )



;f) La edad de todos los INFORMATICOS
(display "La edad de los informaticos: ")
(Extrae Datos (gen-filtro (lambda (x) (get-estudios x)) member 'INFORMATICA) get-edad)



;g) La lista de nombres completos de todas las personas sin trabajo
(display "Nombres completos sin trabajo: ")
(Extrae Datos (gen-filtro (lambda (x) (trabaja? x)) equal? #f) get-nombre-completo)
